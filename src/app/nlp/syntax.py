...
opening = ("As with most of the concepts described here, syntax and grammar "
           ...
           "where you and I can communication very basic ideas such as these "
...
structure = ("The tiniest unit of 'Basic Text' are `characters`.  Depending on"
             " the computer encoding, that can range from a very limited "
             "selection, such as ASCII (256 chars), to the more robust UTF-8 "
             "(or UTF-16) encoding which represents upwards of 1,112,064 "
             "characters.\nThe next level of structure builds the idea of "
             "`words`.  A word is any combination of characters that is not "
             "whitespace.  There are quite a bit of complexities that arise "
             "from this definition.  To name a few,")
...
structure2 = [
    {"key": 1, "value": (" A period (.) can be used as an abbreviation marker "
                         ...
                         "mark the next level of `sentences`.")},
    {"key": 2, "value": ("Special characters can end up in `words`, such as "
                         "(AT&T, T-Mobile, Chips Ahoy!, 5,000, 2K, ...) ")},
    {"key": 3, "value": (" Proper names can represent one \"`word`\" with "
                         "spaces, such as (Supreme Court, Walt Disney, United "
                         "States, ...)\n In any case, each space-separated "
                         "`word` has it's own meaning.  'United' is valid.  "
                         "'States' is valid.  The unification falls more into "
                         "the realm of semantics and pragmatics.")}
]

structure3 = ("For all intents and purposes, spaces are the accepted separator"
              ...
              "`sentences` are the next class of unit.  There are a few "
              ...
              "`sentences`'.  As a neat formula, text can be expressed as the "
              "following relationship:\nText = Group(Sentence(Word(Char)))\n"
              "\twhere a `Group` is a combination of 'n' `sentences`,\n"
              "\tand a `Sentence` is a combination of 'n' `words`,\n"
              "\tand a `Word` is a combination of 'n' `chars`\n")
